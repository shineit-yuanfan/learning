...
开始从小的地方全面管理迁移OdooS.A.，可以用两个方法测试Odoo，并用他服务与你的企业。但是限制了系统的灵活性（检查：没有自定义模块？还有呢？）

可以用来检测Odoo和长期“制造”使用。

:ref:`setup/install/packaged`

简单的开始，使得更多灵活性的托管与部署，在控制数据储存的地方更好的控制。解决了了用户的负担。

很容易的测试出Odoo的显影模块和可用于与其他部署和维护工作长期生产使用。

:ref:`setup/install/source`

比之前更复杂的:ref:`setup/install/packaged`提供了更大的灵活性。打包安装程序通常允许多个oDoo程序运行在同一个系统上,Odoo版本不提供简单源访问Odoo本身。

良好的开发模块,可以用作生产基地部署

源代码可以通过下载压缩包或使用git获得。使用git更易于更新,多个版本之间切换(包括当前开发版本)或做出贡献。
...
为了简单的得到Odoo的快速想法，演示实例可用，他们共享实例，在这几个小时里，可以浏览和尝试没有递交的东西。
